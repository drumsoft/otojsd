...
Otojs.filt.lpf_biquad = function () {
  ...
  return (input, frequency, q) => {
    ...
    y1 = y;
    y =
      (b0 * x + b1 * x1 + b0 * x2 + 2 * cs * y1 - (1 - alpha) * y2) /
      (1 + alpha);
  ...
  return (trigger) => {
    ...
    switch (is_on) {
      case true:
        level =
          elapsed <= attack
            ? s_attack * elapsed + start_level // attack phase
            : elapsed < attack + decay
            ? s_decay * (elapsed - attack) + 1 // decay phase
            : sustain; // sustain phase
        break;
      case false:
        level =
          elapsed < release
            ? s_release * elapsed + start_level // release phase
            : 0; // off
        break;
    }
    elapsed += Otojs.dtime;
  ...
  play(tick) {
    ...
    if (this.loop) {
      ...
      if (t < this.prev_tick) {
        ...
        this.next_index = 0;
        this.next_tick =
          this.events.length > 0 ? this.events[0].tick : undefined;
      }
    }
    while (this.next_tick !== undefined && this.next_tick <= t) {
      ...
      this.next_index++;
      this.next_tick =
        this.next_index < this.events.length
          ? this.events[this.next_index].tick
          : undefined;
    }
    this.prev_tick = t;
  }
    ...
    function parse_note(note) {
      ...
      if (note.label in number_by_label || note.label === "N") {
        ...
        if (note.label === "N") {
          ...
          note_number =
            number_by_label[note.label] +
            (octave + 1) * 12 +
            parse_modifier(note.modifier);
