...
kick_mml.score(
  "Q4 L4 O1 G!G!G!G! G!G!G!G! G!G8.!G16G!G! G!G!G!G8!G8",
  null,
  true
);
...
var sc = (note, repeat = 1) =>
  `RR${note}${note}!RR${note}${note}!RR${note}${note}!RR${note}${note}!`.repeat(
    repeat
  );
...
function oto_render(frames, channels, input_array, midi_input) {
  ...
  for (let f = 0; f < frames; f++) {
    ...
    let v0 = overdrive(0.4 * kick(tick - swing));
    let v1 = overdrive(0.7 * bass(tick - swing));
    let v2 = overdrive(0.4 * synth(tick - swing * 0.7));
