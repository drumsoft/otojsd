...
function oto_render(frames, channels, input_array) {
  ...
  for (let f = 0; f < frames; f++) {
    // グローバルLFO（ゆっくりとした揺れ）
    let global_lfo =
      0.5 + 0.5 * Math.sin((2 * Math.PI * frame * 0.2) / sample_rate);
    ...
    let bass_env = Math.max(
      0,
      1 - (frame % (sample_rate / 4)) / (sample_rate / 8)
    );

    // FM変調でベースに倍音を追加
    let bass_mod =
      Math.sin((2 * Math.PI * frame * bass_freq * 3) / sample_rate) *
      bass_env *
      2;
    let bass =
      Math.sin((2 * Math.PI * frame * bass_freq) / sample_rate + bass_mod) *
      bass_env *
      0.3;
    ...
    let arp_env = Math.max(
      0,
      1 - (frame % (sample_rate / 16)) / (sample_rate / 32)
    );

    // アルペジオにビブラート追加
    let vibrato =
      Math.sin((2 * Math.PI * frame * 6) / sample_rate) * 10 * global_lfo;
    let arp =
      Math.sin((2 * Math.PI * frame * (arp_freq + vibrato)) / sample_rate) *
      arp_env *
      0.15;
    ...
    let pad1 =
      Math.sin((2 * Math.PI * frame * 220) / sample_rate + pad_lfo * 0.5) * 0.1;
    let pad2 =
      Math.sin((2 * Math.PI * frame * 330) / sample_rate - pad_lfo * 0.3) *
      0.08;
    ...
    let hat_env = hat_trigger
      ? Math.max(0, 1 - (frame % (sample_rate / 8)) / (sample_rate / 64))
      : 0;
